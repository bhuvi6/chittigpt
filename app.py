    ...
    for chat in history:
        if chat['id'] == chat_id:
            ...
            if chat['title'] == "New Chat":
                try:
                    title_prompt = f"Generate a short 4-6 word title for this conversation based on the user's message: '{user_input}'"
                    title_response = model.generate_content(title_prompt)
                    smart_title = title_response.text.strip().replace("\n", "")
                    chat['title'] = smart_title
                except Exception as e:
                    # fallback to trimmed input
